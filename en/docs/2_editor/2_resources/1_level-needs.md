...
**One resource can list several sources.** A resource carries a list of places it can be fetched from - a file beside the level, an absolute path, a direct url - and the game walks that list until something loads
...
**Folder size.** There is no formal limit, but a level travels by copying, and the folder's weight is what the person receiving it sees. A 4-minute track in `ogg` is 4 to 6 MB, the same track in `wav` is around 40 MB
...
Next: [[2_preparing-the-track|Preparing the track]], [[3_images-and-fonts|Images and fonts]]
