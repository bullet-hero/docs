...
| `advanced_player` | игрок, которому интересны механики |
| `level_author` | делает уровни в редакторе |
| `developer` | строит что-то на SDK или расширяет игру |
| `server_host` | держит сервер для себя и друзей |
| `server_advanced` | крупный владелец сервера, готовый расширить или написать сервер |
| `contributor` | правит тексты и переводы в этом репозитории |

Заметки в `notes/` вместо этого используют тематические теги: `legal`, `meta`, `reference`
