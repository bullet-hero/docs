...
**A segment is a stretch of constant tempo**, and there can be several. Between them there can be holes - an intro with no percussion, a break, the tail after the last note

> [!info] Worth knowing
> That is why this is a list of stretches rather than a track of keys: a key track cannot express absence, and its last point would reach to the end of the level

> [!tip] Recommendation
> Map only the stretches that will carry content. An intro usually needs nothing

> [!tip] Recommendation
> Find the bpm first, then the offset, then the beats per bar. If the bpm is unknown, tap it in on the `T` key, round to a whole number and fix the rest with the offset
...
> [!warning] Warning
> Syncopation gives groove, but used often or inconsistently it stops the player knowing where they are

**Structure.** Sections are what a player remembers. A level without them is four minutes of the same thing even when something new happens every five seconds
