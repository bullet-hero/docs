...
> [!tip] Recommendation
> Put content colour through a theme reference, and use a literal only where the colour is genuinely unique and must follow nothing. The background is themeable too
...
**Post-processing.** The standard URP set plus two glitch effects, all keyframed, so every parameter can be animated
...
**Anti-aliasing.** The default is `MSAA`, which resolves geometric edges exactly - every shape is real geometry. The alternative is `FXAA`, whose cost is one full-screen pass no matter how much the level overdraws. `TAA` and `STP` are not offered, because they leave a trail behind fast-moving objects, and a trail behind a projectile is a readability problem

> [!info] Worth knowing
> That is the player's setting rather than yours. A level that only looks right under one mode does not look right for everyone
