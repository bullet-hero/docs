...
**Numbers first, because difficulty cannot be discussed without them.** The player's avatar is drawn `0.5` wide, but its hitbox is a circle of only `0.15` in radius. Ordinary speed is `15` per second. The dash is `50` per second, lasts `0.15 s` and recharges over `0.3 s`, so one dash is `7.5` units covered almost instantly, and you are invulnerable for the whole `0.3 s`
...
**A hit does not kill immediately.** After being hit the player is knocked back and has no control for `0.2 s`, and every further hit is ignored for a full `1 s`. A wall of ten projectiles in a row costs one life, not ten
...
> [!caution] Caution
> Lives, speed and checkpoints are the player's choice. You can place checkpoints, you cannot make anyone use them, and you cannot forbid playing at half speed. What you design is a sensation, not a punishment

**The curve: teach, test, twist.** Show the pattern first in a safe form where failing is nearly impossible. Then demand it for real. Then change one variable - speed, direction, count - and demand it again
...
> [!tip] Tip
> A level passable only from memory is a legitimate genre. The line is that after dying, the player has to understand what there is to memorise. Still unclear after the tenth death is not memorisation, it is noise
