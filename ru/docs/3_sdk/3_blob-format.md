...
- **поколение кодека** в заголовке описывает раскладку байтов. Когда оно меняется, каждый старый `.blob` становится нечитаемым целиком: откатиться не к чему. Оно выросло с 1 до 2, когда конверт перестал хранить два числа `ushort` и начал хранить одно `int`
