...
The resource already carries terms as free as `CC BY-NC` or freer. It is not converted into `CC BY-NC`, it keeps its own
...
A private "sure, go ahead" is not enough on its own: a level under `CC BY-NC` is redistributed publicly, so the permission has to cover that rather than your personal use
